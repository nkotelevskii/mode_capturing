...
class Gen_network(nn.Module):
    def __init__(self, z_dim, args):
        ...
        if args.decoder == "deconv":
            ...
            if self.size_c == 3:
                self.deconv3 = nn.ConvTranspose2d(in_channels=16, out_channels=self.size_c, kernel_size=7,
                                                  stride=2, padding=1, output_padding=1)
                self.deconv4 = nn.ConvTranspose2d(in_channels=16, out_channels=self.size_c, kernel_size=7,
                                                  stride=2, padding=1, output_padding=1)
            else:
            ...
            self.deconv3 = nn.Sequential(nn.UpsamplingBilinear2d(size=(self.size_h, self.size_w)),
                                         nn.Conv2d(in_channels=16, out_channels=args.data_c, kernel_size=1))
            if self.size_c == 3:
                self.deconv4 = nn.Sequential(nn.UpsamplingBilinear2d(size=(self.size_h, self.size_w)),
                                             nn.Conv2d(in_channels=16, out_channels=args.data_c, kernel_size=1))
    ...
    def forward(self, x):
        ...
        if self.size_c == 3:
            mu = self.deconv3(h4)
            sigma = F.softplus(self.deconv4(h4))
            return [mu, sigma]
        else:
